...
namespace App.Controllers {
    ...
    public class ViewController {
        ...
        public ViewController (AppController controler) {
            ...
            // sharing view
            SharingView view_sharing = new SharingView(controler);
            this.register_view (view_sharing);
            // error view
            ErrorView view_error = new ErrorView(controler);
            this.register_view (view_error);
            // conf view
            ViewConf view_conf = new ViewConf(controler);
            this.register_view (view_conf);
       ...
       public void set_registered_view(string identifier) {
        ...
        public void update_views() {
            ...
            if (this.views_stack.size > 1) {
                ...
                if (this.get_current_view().get_id() != "config") app_controller.window.headerbar.back_button.set_label (_("Back"));
            } else {
